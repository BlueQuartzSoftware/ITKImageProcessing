...
FFTAmoebaOptimizer::FFTAmoebaOptimizer()
: m_InitialSimplexDelta(1)
{
  this->m_MaximumNumberOfIterations = 500;
  this->m_FractionalTolerance = 1e-5;
...
const std::string FFTAmoebaOptimizer::GetStopConditionDescription() const
{
  return this->m_StopConditionDescription.str();
}

void FFTAmoebaOptimizer::PrintSelf(std::ostream& os, Indent indent) const
{
  Superclass::PrintSelf(os, indent);
  os << indent << "MaximumNumberOfIterations: " << this->m_MaximumNumberOfIterations << std::endl;
  os << indent << "FractionalTolerance: " << this->m_FractionalTolerance << std::endl;
...
FFTAmoebaOptimizer::MeasureType FFTAmoebaOptimizer::GetValue() const
{
...
/** Get the Optimizer */
fft_amoeba* FFTAmoebaOptimizer::GetOptimizer() const
{
  return this->m_VnlOptimizer;
}

void FFTAmoebaOptimizer::SetInitialSimplexDelta(ParametersType initialSimplexDelta, bool automaticInitialSimplex)
{
...
void FFTAmoebaOptimizer::SetCostFunction(SingleValuedCostFunction* costFunction)
{
...
void FFTAmoebaOptimizer::StartOptimization()
{
  ...
  // new one
  delete m_VnlOptimizer;
  m_VnlOptimizer = new fft_amoeba(*adaptor);
  m_VnlOptimizer->set_max_iterations(static_cast<int>(m_MaximumNumberOfIterations));
  m_VnlOptimizer->set_f_tolerance(m_FractionalTolerance);
  ...
  if(m_Cancel)
  {
    this->m_StopConditionDescription << "Operation cancelled after " << this->m_VnlOptimizer->get_num_evaluations() << " iterations.";
  }
  else if(static_cast<unsigned int>(this->m_VnlOptimizer->get_num_evaluations()) < this->m_MaximumNumberOfIterations)
  {
    this->m_StopConditionDescription << "Fractional tolerance (" << this->m_FractionalTolerance << ") has been met in " << this->m_VnlOptimizer->get_num_evaluations() << " iterations.";
  }
  else
  {
...
void FFTAmoebaOptimizer::Cancel()
{
  m_Cancel = true;
  m_VnlOptimizer->cancel();
}

void FFTAmoebaOptimizer::SetSIMPLFilter(AbstractFilter* filter)
{
...
void FFTAmoebaOptimizer::ValidateSettings()
{
  ...
  if(this->m_FractionalTolerance < 0)
  {
    itkExceptionMacro(<< "negative fractional tolerance")
  }
}
