...
public:
  ...
  int TestITKCastImagefloatTodoubleTest()
  {
    ...
    WriteImage("ITKCastImagefloatTodouble.nrrd", containerArray, output_path);
  ...
  int TestITKCastImageFailTest()
  {
    ...
    filter->setDataContainerArray(containerArray);
    filter->execute();
    DREAM3D_REQUIRED(filter->getErrorCode(), ==, -5);
    DREAM3D_REQUIRED(filter->getWarningCode(), >=, 0);
    return 0;
  }
...
private:
  ITKCastImageTest(const ITKCastImageTest&) = delete; // Copy Constructor Not Implemented
  void operator=(const ITKCastImageTest&) = delete;   // Move assignment Not Implemented
};
