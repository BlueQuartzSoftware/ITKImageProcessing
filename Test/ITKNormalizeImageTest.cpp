...
public:
  ...
  int TestITKNormalizeImagedefaultsTest()
  {
    ...
    QString filtName = "ITKNormalizeImage";
    ...
    WriteImage("ITKNormalizeImagedefaults.nrrd", containerArray, output_path);
    QString baseline_filename = UnitTest::DataDir + QString("/Data/JSONFilters/Baseline/BasicFilters_NormalizeImageFilter_defaults.nrrd");
  ...
  int TestITKNormalizeImagevectorTest()
  {
    ...
    DREAM3D_REQUIRED(filter->getErrorCondition(), >=, 0);
    DREAM3D_REQUIRED(filter->getWarningCondition(), >=, 0);
    WriteImage("ITKNormalizeImagevector.nrrd", containerArray, output_path);
    QString baseline_filename = UnitTest::DataDir + QString("/Data/JSONFilters/Baseline/BasicFilters_NormalizeImageFilter_vector.nrrd");
    DataArrayPath baseline_path("BContainer", "BAttributeMatrixName", "BAttributeArrayName");
    this->ReadImage(baseline_filename, containerArray, baseline_path);
    int res = this->CompareImages(containerArray, output_path, baseline_path, 0.0001);
    DREAM3D_REQUIRE_EQUAL(res, 0);
...
private:
  ITKNormalizeImageTest(const ITKNormalizeImageTest&) = delete; // Copy Constructor Not Implemented
  void operator=(const ITKNormalizeImageTest&) = delete;        // Operator '=' Not Implemented
};
