...
public:
  ...
  int TestITKSaltAndPepperNoiseImage2dTest()
  {
    ...
    WriteImage("ITKSaltAndPepperNoiseImage2d.nrrd", containerArray, output_path);
    ...
    int res = this->CompareImages(containerArray, output_path, baseline_path, 20);
  ...
  int TestITKSaltAndPepperNoiseImage3dTest()
  {
    ...
    DREAM3D_REQUIRED(filter->getErrorCondition(), >=, 0);
    DREAM3D_REQUIRED(filter->getWarningCondition(), >=, 0);
    WriteImage("ITKSaltAndPepperNoiseImage3d.nrrd", containerArray, output_path);
    ...
    DataArrayPath baseline_path("BContainer", "BAttributeMatrixName", "BAttributeArrayName");
    this->ReadImage(baseline_filename, containerArray, baseline_path);
    int res = this->CompareImages(containerArray, output_path, baseline_path, 10000.0);
    DREAM3D_REQUIRE_EQUAL(res, 0);
    return 0;
  }
  ...
  int TestITKSaltAndPepperNoiseImagergbTest()
  {
    ...
    DREAM3D_REQUIRED(filter->getErrorCondition(), >=, 0);
    DREAM3D_REQUIRED(filter->getWarningCondition(), >=, 0);
    WriteImage("ITKSaltAndPepperNoiseImagergb.nrrd", containerArray, output_path);
